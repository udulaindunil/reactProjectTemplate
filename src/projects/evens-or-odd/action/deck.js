...
export const fetchNewDeck = ()=> dispatch =>{
    ...
    // return fetch('https://deckofcardsapi.com/api/deck/new/shuffle/asas')
    return fetch(`${API_ADDRESS}/deck/new/shuffle`)
        .then(response => {
            if(response.status !== 200){
                throw new Error('Unccessful requset to deskofcardsapii.com');
                }
...
export const fetchDrawCard = deck_id => dispatch =>{
    ...
    .then(json =>dispatch({type:DECK_DRAW.FETCH_SUCCESS,cards: json.cards,remaining: json.remaining}))
